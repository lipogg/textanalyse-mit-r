...
In der Sitzung zum Preprocessing haben wir das Paket UDPipe bereits kennengelernt und UDPipe-Funktionen zur Lemmatisierung unseres Beispielkorpus verwendet. Die Funktion `udpipe()` erzeugt einen Dataframe, der neben den Lemmata auch weitere Spalten mit Daten zu den Tokens in unserem Korpus enthält, zum Beispiel `upos`, `xpos` und `dep_rel`. Derselbe Dataframe kann auch wie in den Abschnitten Textanalyse II und Textanalyse III durch Kombination der Funktionen `udpipe_annotate()` und `as.data.frame()` erzeugt werden. Zur Erinnerung:  
...
Zunächst lesen wir unsere Märchen genau wie im Kapitel "Textanalyse III" ein und erstellen ein Quanteda-Tokens-Objekt. 
...
Anschließend können wir die Funktionen `udpipe()` oder `udpipe_annotate()` (in diesem Fall in Kombination mit der Funktion `as.data.frame()`) verwenden, um das POS Tagging und Dependency Parsing durchzuführen. Aber Achtung: wie wir bereits im Kapitel Textanalyse II besprochen haben, kann die `udpipe()`-Funktion nicht auf bereits mit Quanteda tokenisierte Objekte angewendet werden:    
...
udpipe(maerchen_toks_raw, ud_model)
...
saveRDS(maerchen_annotated, file="data/maerchen_udpipe.rds")
```
...
maerchen_annotated <- readRDS(file="data/maerchen_udpipe.rds")
...
udpipe(maerchen_alle, ud_model, parallel.cores = 4)
...
* Head von "Königstochter" ist "war" (s. Spalte `head_token_id`)
...
* Head von "Königstochter" ist "hatte"
