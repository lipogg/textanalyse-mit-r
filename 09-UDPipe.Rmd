...
Tatsächlich führt die Funktion `udpipe()` nicht nur eine Lemmatisierung durch, sondern auch Part of Speech Tagging und Dependency Parsing, und der Dataframe, den die Funktion zurückgibt, enthält die Ergebnisse dieser Verfahren. In den folgenden beiden Abschnitten werden wir uns deswegen etwas genauer mit diesen Daten beschäftigen.  Im Abschnitt "Part of Speech Tagging mit UDPipe" betrachten wir zunächst die beiden Spalten `upos` und `xpos`:
...
x <- udpipe(maerchen_toks_raw, ud_model)

```

Beim Versuch, die Funktion `udpipe()` auf unser Quanteda tokens-Objekt anzuwenden, bekommen wir eine Fehlermeldung, die uns mitteilt, dass `udpipe()` nicht auf ein bereits tokenisiertes Objekt angewandt werden kann. In den [udpipe- Dokumentationsseiten](https://bnosac.github.io/udpipe/docs/doc2.html#my-text-data-is-already-tokenised) können wir nachlesen, wie wir POS Tagging und Dependency Parsing für ein bereits tokenisiertes Korpus durchführen können: Anstelle der Funktion `udpipe()` muss dafür die Funktion `udpipe_annotate()` verwendet werden, und zwar mit den Argumenten `tagger="default"` (für das POS Tagging),  `parser="default"` (für das Dependency Parsing) und `tokenizer = "vertical"` (damit die Tokens beibehalten werden). Die [udpipe- Dokumentationsseiten](https://bnosac.github.io/udpipe/docs/doc2.html#my-text-data-is-already-tokenised) geben außerdem die folgende Codezeile (die mit sapply()) zur Vorbereitung des Tokens-Objekts für die Annotation mit udpipe an:  

```{r eval=FALSE}
# Quanteda tokens-Objekt für die Verarbeitung mit der Funktion udpipe_annotate() vorbereiten
maerchen_txt <- sapply(maerchen_toks_raw, FUN=function(x) paste(x, collapse = "\n"))

# Lemmatisierung, POS-Tagging und Dependency Parsing mit udpipe_annotate()
maerchen_annotated <- udpipe_annotate(ud_model, maerchen_txt, tagger="default", parser="default", tokenizer = "vertical", doc_id = maerchen_toks_raw$Titel)

# Annotiertes Korpus zwischenspeichern
saveRDS(maerchen_annotated, file="data/maerchen_annotated.rds")
```

Die Annotation eines gesamten Korpus dauert je nach Rechenleistung des Computers sehr lange. Es ist deswegen ratsam, das annotierte Korpus zwischenzuspeichern, damit es nicht für jede RStudio Sitzung neu erzeugt werden muss. Wir speichern unser Objekt `maerchen_annotated` als RDS-Datei, ein R-internes Datenformat, das zum Zwischenspeichern von R-Objekten verwendet werden kann (s. [Abschnitt 5.8](file:///Users/lipogg/Desktop/LV_Einfuehrung_WS2324/einfuehrung-in-r/docs/textanalyse-i-korpus-tokens-daten-und-dateien.html#daten-schreiben)). RDS-Dateien können in eine R Sitzung eingelesen werden mit: 


```{r}
# So kann das annotierte Korpus eingelesen werden 
maerchen_annotated <- readRDS(file="data/maerchen_annotated.rds")

```

Zuletzt muss das annotierte Korpus noch in einen Dataframe umgewandelt werden:
...
Mithilfe der POS Tags in der Spalte `upos` können wir zum Beispiel eine **Token-Häufigkeitsanalyse auf bestimmte Wortarten eingrenzen**. Wir wiederholen beispielhaft die Token-Häufigkeitsanalyse aus der vergangenen Woche für alle Substantive in unserem Märchenkorpus:
...
substantive_dfm <- dfm(substantive)
...
* Head von "Königstochter" ist "schön" (s. Spalte `head_token_id`)
* `dep_rel` von "Königstochter" ist `nsubj`, "schön" ist `root` 

"Die schöne Königstochter ging in den Wald."

* Head von "schön" ist "Königstochter", aber Head von "Königstochter" ist "ging"
...
Bei der Interpretation der Ergebnisse sollten wir die Performance des gewählten Modells sowie die Art und Qualität der Texte in unserem Korpus berücksichtigen. Die Ergebnisse waren nicht fehlerfrei und wir können davon ausgehen, dass die Ergebnisse der Verfahren für unsere Märchentexte um einiges schlechter sein werden als die angegebenen Prozentzahlen für das Testkorpus, da unsere Texte sich im Inhalt, in der Textgattung und auch im historischen Kontext von den Texten aus dem Trainingskorpus unterscheiden. Genau aus diesem Grund gibt es in den Digital Humanities viele Bestrebungen, domänenspezifische Modelle zu entwickeln, um die Ergebnisse für historische und literarische Texte zu verbessern. 
