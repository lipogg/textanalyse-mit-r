...
Wir haben also jetzt einen Dataframe, der chronologisch alle Charaktere und deren Sprechtexte enthält. Um herauszufinden, aus wie vielen Tokens der Sprechtext der Charaktere insgesamt besteht, müssen wir zunächst die einzelnen Sprechtexte zu einer langen Zeichenkette zusammenfügen. Wir müssen also den Dataframe so umformen, dass jede Zeile einem Charakter entspricht und in der Spalte `text` der gesamte Sprechtext dieses Charakters steht. Eine ähnliche Operation haben wir bereits in der Sitzung zum Preprocessing kennengelernt, und zwar im Abschnitt 7.7 "Lemmatisierung":
...
Zur Erinnerung: Dieser Code hat für jede der Dokumenten-IDs in der Spalte `doc_id` in dem Dataframe `kafka_cleaned_df` die Lemmata zu einer einzigen Zeichenkette zusammengefügt. 

Wir wollen mit unserem Dataframe speakers_df etwas ganz ähnliches machen, nämlich für jeden Charakter in der Spalte `speaker` den Sprechtext in der Spalte `text` in einer Zeichenkette zusammenfassen:

```{r warning=FALSE, message=FALSE, attr.output='style="max-height: 200px;"'}
# Dataframe so umformen, dass jede Zeile einem Charakter entspricht und in der Spalte text der gesamte Sprechtext dieses Charakters steht

library(dplyr)

speakers_total <- speakers_df %>%
  group_by(speaker) %>%
  summarise(text = paste(text, collapse="\n"))
speakers_total # View(speakers_total)
```
...
library(quanteda)
speakers_cor <- corpus(speakers_total)
...
humboldt_map <- ggplot(data = map_data) +
  geom_sf() +
  geom_point(data = site_data, aes(x = longitude, y = latitude, text=name), size = 0.8, 
             shape = 21, fill = "darkred") + # shape = 20, color = "darkred"
  theme_minimal() +
