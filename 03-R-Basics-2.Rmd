...
begriffe <- c("Element", "Index", "Indexierung", "Zugreifen", "Vektorisierung", "Dimension")
englisch <- c("Element", "Index", "Indexing", "Access", "Vectorization", "Dimension")
definitionen <- c(
  "Wert oder anderes Objekt innnerhalb einer Datenstruktur. Datenstrukturen können selbst Elemente einer anderen Datenstruktur sein. Elemente haben einen Index und manchmal zusätzlich einen Namen (oder 'Schlüssel'), über den auf das Element zugegriffen werden kann.",
  "Position eines Elements innerhalb einer Datenstruktur. Ein Index ist in R immer eine Zahl >= 1.",
  "Überbegriff für verschiedene Operationen, mit denen auf Elemente in einer Datenstruktur zugegriffen werden kann.",
  "Operation, bei der durch den Einsatz spezifischer Operatoren Elemente einer Datenstruktur ausgewählt, diese extrahiert oder modifiziert werden können.",
  "Mechanismus, bei dem Funktionen oder Operationen automatisch auf alle Elemente eines Vektors angewandt werden, statt auf einzelne Elemente.",
  "In Bezug auf Matrizen die Anzahl von Zeilen und Spalten (angegeben als Zeilen x Spalten). In Bezug auf Arrays allgemein die Anzahl der verschiedenen 'Richtungen', in denen Daten organisiert sind. Bei einem Array können das nicht nur Zeilen und Spalten sein, sondern auch 'Schichten' von Matrizen und andere Organisationsweisen." 
  )  
...
knitr::include_graphics("images/vektor.png")
```
...
```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("images/liste-neu.png")
```
...
```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("images/binding-1.png")
```
