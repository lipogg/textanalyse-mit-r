...
<img src="08-Textanalyse-3_files/figure-html/unnamed-chunk-69-1.png" width="672" />
...
```r
collocs_1 <- textstat_collocations(maerchen_corpus, min_count = 1)
subset_collocs <- collocs_1[grepl("schön\\w*", collocs_1$collocation), ]
subset_collocs
```
...
```{style="max-height: 200px;"}
##               collocation count count_nested length   lambda         z
## 111              so schön    55            0      2 4.473393 22.761722
## 372       schöne jungfrau    11            0      2 5.284236 16.263218
## 395           schöner als    17            0      2 4.927963 16.021252
## 700           ein schönes    33            0      2 6.288886 13.654563
## 720           schönste im     9            0      2 5.555333 13.547804
## 1013 schöne königstochter     8            0      2 4.442722 12.212096
## 2204         die schönste    22            0      2 5.083194  9.840374
```
...
Aber wenn ein Paket nicht genau die Funktionalität bietet, die wir brauchen, können wir einfach unsere eigene Funktion schreiben. In diesem Abschnitt werden wir eine Funktion mit dem Namen `fcm_pmi()` definieren, welche eine Feature-Co-occurrence-Matrix (FCM) als Argument annimmt, für jeden Wert in der Matrix ein Assoziationsmaß berechnet, das Pointwise Mutual Information genannt wird, und als Rückgabewert die auf diese Weise gewichtete FCM zurückgibt. Die Funktion wird parallel zur Quanteda-Funktion dfm_tfidf() benannt und der Code ist adaptiert von https://github.com/quanteda/quanteda/issues/2149.
...
Dieses Maß hat einen informationstheoretischen und wahrscheinlichkeitstheoretischen Hintergrund (s. optionaler Abschnitt "Mathematischer Hintergrund: PMI"). 

Zur Berechnung der PPMI-Werte verwenden wir die folgende Funktion von Kohei Watanabe, einem der Entwickler des Quanteda-Pakets: 


```r
fcm_pmi <- function(x) {
  m <- x@meta$object$margin
  x <- as(x, "TsparseMatrix") # im Original steht hier "dgTMatrix"
...
Bevor wir die Funktion auf unser Korpus anwenden, betrachten wir ein Beispiel: 
...
ggplot(data = cooc_df, aes(x = factor(nrow(cooc_df):1), y = count)) +
  ...
  scale_x_discrete(breaks = factor(nrow(cooc_df):1), labels = cooc_df$feature) +
  labs(x = NULL, y = "Count") +
  ggtitle("Höchste PMI-Werte für Kookkurrenzen mit 'königstochter'")
```

<img src="08-Textanalyse-3_files/figure-html/unnamed-chunk-79-1.png" width="672" />

:::task

Verständnisfragen: 
- Vergleicht diese Diagramme mit den Diagrammen aus dem Abschnitt "Kookkurrenzen". Was fält euch auf?

:::

Ihr habt gesehen: **Auch der Gewichtung von Kookkurrenzen können unterschiedliche mathematische Zusammenhänge und Auffassungen zugrunde liegen. Die textstat_collocations() Funktion verwendet beispielsweise statistische Hypothesentests, um die Signifikanz von N-Grammen zu bestimmen. Unsere Funktion `fcm_pmi()` zur Gewichtung der Kookkurrenzmatrizen verwendet dagegen Konzepte aus der Informationstheorie (entropy, mutual information), um die Assoziation von zwei Wörtern in einem Kontext zu quantifizieren.** Diese verschiedenen mathematischen Hintergründe müssen bei der Interpretation der Werte berücksichtigt werden. Und wie sieht es mit dem theoretischen Hintergrund aus? Noah Bubenhofer fasst N-Gramme, Kookkurrenzen und Kollokationen als "Sprachgebrauchsmuster" (Bubenhofer 2009) auf, 
