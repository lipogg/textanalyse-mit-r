...
Der Parameter, der uns im Rahmen einer Kollokationsanalyse hauptsächlich interessiert, ist $\lambda$, denn dieser Parameter repräsentiert den Effekt, den das gemeinsame Vorkommen der beiden Wörter auf die Wahrscheinlichkeit für das gemeinsame Auftreten der Wörter hat (den "Interaktionseffekt"). Er wird deswegen auch **Interaktionsparameter** (engl. "interaction parameter") genannt. Aus der log-linearen Formel können wir das $\lambda$ extrahieren, indem nach $\lambda$ umgestellt wird:
