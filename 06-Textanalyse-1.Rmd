...
knitr::include_graphics("images/korpus-tokens-dfm.png")
```
...
interessantheit <- rev(1:10)
interpretierbarkeit <- 1:10
df <- data.frame(interessantheit, interpretierbarkeit)


fig <- plot_ly(df, 
               x = ~interessantheit, 
               y = ~interpretierbarkeit, 
               type = "scatter", 
               mode="lines") %>%
  layout(title = "Interessantheit vs. Interpretierbarkeit",
         xaxis = list(title = "Interessantheit der Analyse"),
         yaxis = list(title = "Interpretierbarkeit der Methode"))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
library(readtext)
ger_texte <- readtext("data/korpus/*.txt", docvarsfrom = "filenames", dvsep = "_", docvarnames = c("Autor_in", "Titel", "Jahr"), encoding = "UTF-8")
```
